...
#         # Initialize DgraphClient if provided
...
#             )
#             self.logger.info(
#                 f"Chroma initialized successfully with collection: {self.collection_name}"
#             )
#         except Exception as e:
...
#             # Also add embeddings to Dgraph if client is available
#             if self.dgraph_client:
#                 for contract, embedding in zip(contracts, embeddings):
#                     try:
#                         self.dgraph_client.insert_embeddings(
#                             contract.get("ContractDeployment.id"), embedding
#                         )
#                     except Exception as e:
#                         self.logger.error(
#                             f"Failed to insert embedding to Dgraph for contract {contract.get('uid')}: {str(e)}"
#                         )
...
#         # Initialize DgraphClient
#         dgraph_client = DgraphClient()

#         # Pass DgraphClient to VectorDBManager
#         manager = VectorDBManager("config/vectordb.yaml", dgraph_client=dgraph_client)
#         logger.info("VectorDBManager initialized.")

#         logger.info("Adding embeddings...")
#         manager.add_embeddings(contracts)
#         logger.info("Embeddings added successfully to both vector store and Dgraph.")
...
#             "Open-source ERC-20 token contract with mint/burn functions and snapshot capabilities.",
#             k=2,
#         )
